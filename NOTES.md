...
CSF-filled and dark, gyral crowns are grey matter and brighter, so a lateral sweep
...
centre, but a protocol amendment that slips to November would leave the team with
...
mode would take an actual x/y/z in millimetres and give all three axes at once,
...
- Superior-inferior is still unmapped, and there is nothing obvious left to map
  it to. Pitch is intensity, rate is density, colour is now front-back. A
